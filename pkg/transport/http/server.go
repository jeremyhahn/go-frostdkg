...
func (s *HTTPServer) handleJoinSession(w http.ResponseWriter, r *http.Request, sessionID string) {
	...
	s.logger.Info("Participant %d joined (active: %d/%d)", participantIdx, len(s.participants), s.sessionConfig.NumParticipants)
...
func (s *HTTPServer) handleRound1Post(w http.ResponseWriter, r *http.Request, sessionID string) {
	...
	s.logger.Debug("Received Round1 from participant %d (%d/%d)", participantIdx, len(s.round1Data), s.sessionConfig.NumParticipants)
...
func (s *HTTPServer) handleRound2Post(w http.ResponseWriter, r *http.Request, sessionID string) {
	...
	s.logger.Debug("Received Round2 from participant %d (%d/%d)", participantIdx, len(s.round2Data), s.sessionConfig.NumParticipants)
...
func (s *HTTPServer) handleCertEq(w http.ResponseWriter, r *http.Request, sessionID string) {
	...
	s.logger.Debug("Received CertEq signature from participant %d (%d/%d)", participantIdx, len(s.certEqSigs), s.sessionConfig.NumParticipants)
