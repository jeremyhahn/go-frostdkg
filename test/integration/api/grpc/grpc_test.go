...
// TestGRPC_2of3_Threshold tests 2-of-3 threshold DKG using gRPC transport.
func TestGRPC_2of3_Threshold(t *testing.T) {
	const (
		threshold       = 2
		numParticipants = 3
	...
	coord, address, err := api.CreateGRPCCoordinator(sessionID, threshold, numParticipants, ciphersuite, certs)
	if err != nil {
		t.Fatalf("Failed to create coordinator: %v", err)
	}
	cleanup.AddCoordinator(coord)

	participants, err := api.CreateGRPCParticipants(numParticipants, certs, ciphersuite)
	if err != nil {
	...
	t.Logf("gRPC transport 2-of-3 DKG completed successfully")
}

// TestGRPC_3of5_Threshold tests 3-of-5 threshold DKG using gRPC transport.
func TestGRPC_3of5_Threshold(t *testing.T) {
	const (
		threshold       = 3
		numParticipants = 5
		ciphersuite     = "FROST-ED25519-SHA512-v1"
	...
	t.Logf("gRPC transport 3-of-5 DKG completed successfully")
}

// TestGRPC_ErrorCases tests error handling with gRPC transport.
func TestGRPC_ErrorCases(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("InvalidThreshold", func(t *testing.T) {
		...
		sessionID := api.GenerateSessionID()
		_, _, err = api.CreateGRPCCoordinator(sessionID, 0, 3, "FROST-ED25519-SHA512-v1", certs)
		if err == nil {
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		sessionID := api.GenerateSessionID()
		coord, _, err := api.CreateGRPCCoordinator(sessionID, 2, 3, "FROST-ED25519-SHA512-v1", certs)
		if err != nil {
			t.Fatalf("Failed to create coordinator: %v", err)
		}
		cleanup.AddCoordinator(coord)
