...
// sendMessage sends a message to the coordinator.
//
//nolint:unused // Used by integration tests with build tag
func (pp *P2PParticipant) sendMessage(msgType transport.MessageType, payload interface{}) error {
	if !pp.connected.Load() {
	...
	// Serialize envelope with payload
	data, err := pp.serializer.MarshalEnvelope(
		pp.sessionID,
		msgType,
		pp.participantIdx,
		payload,
...
// receiveMessage waits for a message of the expected type.
//
//nolint:unused // Used by integration tests with build tag
func (pp *P2PParticipant) receiveMessage(ctx context.Context, expectedType transport.MessageType, timeout time.Duration) (*transport.Envelope, error) {
	if !pp.connected.Load() {
		return nil, transport.ErrNotConnected
	}
	...
	for {
		...
		case envelope := <-pp.messageChan:
			...
			// Wrong type, continue waiting
		case err := <-pp.errorChan:
			return nil, err
		}
	}
}
