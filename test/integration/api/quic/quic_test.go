...
// TestQUIC_2of3_Threshold tests 2-of-3 threshold DKG using QUIC transport.
func TestQUIC_2of3_Threshold(t *testing.T) {
	const (
		threshold       = 2
		numParticipants = 3
	...
	coord, address, err := api.CreateQUICCoordinator(sessionID, threshold, numParticipants, ciphersuite, certs)
	if err != nil {
		t.Fatalf("Failed to create coordinator: %v", err)
	}
	cleanup.AddCoordinator(coord)

	participants, err := api.CreateQUICParticipants(numParticipants, certs, ciphersuite)
	if err != nil {
	...
	t.Logf("QUIC transport 2-of-3 DKG completed successfully")
}

// TestQUIC_3of5_Threshold tests 3-of-5 threshold DKG using QUIC transport.
func TestQUIC_3of5_Threshold(t *testing.T) {
	const (
		threshold       = 3
		numParticipants = 5
		ciphersuite     = "FROST-ED25519-SHA512-v1"
	...
	t.Logf("QUIC transport 3-of-5 DKG completed successfully")
}

// TestQUIC_ErrorCases tests error handling with QUIC transport.
func TestQUIC_ErrorCases(t *testing.T) {
	ctx := context.Background()
	...
	t.Run("InvalidThreshold", func(t *testing.T) {
		...
		sessionID := api.GenerateSessionID()
		_, _, err = api.CreateQUICCoordinator(sessionID, 0, 3, "FROST-ED25519-SHA512-v1", certs)
		if err == nil {
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		sessionID := api.GenerateSessionID()
		coord, _, err := api.CreateQUICCoordinator(sessionID, 2, 3, "FROST-ED25519-SHA512-v1", certs)
		if err != nil {
			t.Fatalf("Failed to create coordinator: %v", err)
		}
		cleanup.AddCoordinator(coord)
