...
func TestRootCommand(t *testing.T) {
	...
	expectedCommands := []string{"version", "coordinator", "participant", "keygen", "verify", "config"}
...
// TestKeygenFlags tests keygen command flags
func TestKeygenFlags(t *testing.T) {
	tests := []struct {
		name     string
		flagName string
	}{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			flag := keygenCmd.Flags().Lookup(tt.flagName)
			require.NotNil(t, flag, "flag %s not found", tt.flagName)
		})
	}
}
...
// TestKeygenECDSA tests ECDSA key generation
func TestKeygenECDSA(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ecdsa"
	keygenOutput = tmpDir
	keygenName = "test-ecdsa"
	keygenDays = 365
	keygenHosts = []string{"localhost", "127.0.0.1"}
	verbose = false
...
// TestKeygenEd25519 tests Ed25519 key generation
func TestKeygenEd25519(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ed25519"
	keygenOutput = tmpDir
	keygenName = "test-ed25519"
	keygenDays = 30
	keygenHosts = []string{"test.example.com"}
	verbose = false
...
// TestKeygenEd25519Verbose tests Ed25519 key generation with verbose flag
func TestKeygenEd25519Verbose(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ed25519"
	keygenOutput = tmpDir
	keygenName = "test-ed25519-verbose"
	keygenDays = 30
	keygenHosts = []string{"test.example.com"}
	verbose = true
...
// TestKeygenInvalidType tests keygen with invalid key type
func TestKeygenInvalidType(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "rsa" // invalid
	keygenOutput = tmpDir
	keygenName = "test"
	keygenDays = 365
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenInvalidDays tests keygen with invalid days
func TestKeygenInvalidDays(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ecdsa"
	keygenOutput = tmpDir
	keygenName = "test"
	keygenDays = 0 // invalid
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenDirectoryCreationError tests keygen with invalid directory path
func TestKeygenDirectoryCreationError(t *testing.T) {
	// Use /dev/null which cannot have directories created inside it
	keygenType = "ecdsa"
	keygenOutput = "/dev/null/cannot/create/dirs"
	keygenName = "test"
	keygenDays = 365
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenWithIPAddress tests keygen with IP address in hosts
func TestKeygenWithIPAddress(t *testing.T) {
	tmpDir := t.TempDir()
	...
	keygenName = "test-ip"
	keygenDays = 100
	keygenHosts = []string{"192.168.1.1", "10.0.0.1", "example.com"} // Mix of IPs and DNS names
	verbose = true
...
// TestKeygenECDSAFileWritePermissions verifies cert file permissions
func TestKeygenECDSAFileWritePermissions(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ecdsa"
	keygenOutput = tmpDir
	keygenName = "test-perms"
	keygenDays = 365
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenEd25519WithSingleHost tests Ed25519 with single host
func TestKeygenEd25519WithSingleHost(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ed25519"
	keygenOutput = tmpDir
	keygenName = "single-host"
	keygenDays = 180
	keygenHosts = []string{"single.example.com"}
	verbose = false
...
// TestKeygenECDSAWithLongValidity tests ECDSA with long validity period
func TestKeygenECDSAWithLongValidity(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ecdsa"
	keygenOutput = tmpDir
	keygenName = "long-validity"
	keygenDays = 3650 // 10 years
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenAllPaths tests all code paths in keygen
func TestKeygenAllPaths(t *testing.T) {
	tests := []struct {
		name    string
		keyType string
		days    int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			keygenType = tt.keyType
			keygenOutput = tmpDir
			keygenName = tt.name
			keygenDays = tt.days
			keygenHosts = tt.hosts
			verbose = tt.verbose
...
// TestKeygenCertFileWriteError tests certificate file write error
func TestKeygenCertFileWriteError(t *testing.T) {
	...
	keygenName = "test2" // Different name to avoid existing file
	err = runKeygen(keygenCmd, nil)
...
// TestKeygenKeyFileWriteError tests key file write error specifically
func TestKeygenKeyFileWriteError(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ecdsa"
	keygenOutput = tmpDir
	keygenName = "write-error-test"
	keygenDays = 365
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenEd25519KeyFileWriteError tests Ed25519 key file write error
func TestKeygenEd25519KeyFileWriteError(t *testing.T) {
	tmpDir := t.TempDir()

	keygenType = "ed25519"
	keygenOutput = tmpDir
	keygenName = "ed25519-write-error"
	keygenDays = 365
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenDefaultValues tests keygen command default values
func TestKeygenDefaultValues(t *testing.T) {
	typeFlag := keygenCmd.Flags().Lookup("type")
	require.NotNil(t, typeFlag)
	assert.Equal(t, "ecdsa", typeFlag.DefValue)

	outputFlag := keygenCmd.Flags().Lookup("output")
	require.NotNil(t, outputFlag)
	assert.Equal(t, "./certs", outputFlag.DefValue)

	daysFlag := keygenCmd.Flags().Lookup("days")
...
// TestKeygenNegativeDays tests keygen with negative days
func TestKeygenNegativeDays(t *testing.T) {
	keygenType = "ecdsa"
	keygenOutput = t.TempDir()
	keygenName = "test"
	keygenDays = -1
	keygenHosts = []string{"localhost"}
	verbose = false
...
// TestKeygenCmdUsage tests keygen command usage string
func TestKeygenCmdUsage(t *testing.T) {
	assert.Equal(t, "keygen", keygenCmd.Use)
	assert.NotEmpty(t, keygenCmd.Short)
	assert.NotEmpty(t, keygenCmd.Long)
}
